...
func decide() -> EnemyState:
	...
	if blackboard.target:
		...
		if not on_cooldown and blackboard.distance_to_target <= attack_state.attack_range:
			return attack_state
		if on_cooldown and blackboard.distance_to_target >= advance_state.max_retreat_distance:
			return es_idle
